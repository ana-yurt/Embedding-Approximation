...
def process_year(target, year, directory, window_size=10, top_n=2000, output_dir="/home/maxzhuyt/Desktop/coha/context_counts"):
    ...
    for filename in os.listdir(directory):
        ...
        if year_pattern.match(filename):
            ...
            # Find all contexts for the target word within the specified window
    ...
    output_file = os.path.join(output_dir, f"context_counts_{target}_{year}.csv")
    year_df.to_csv(output_file)
    logging.info(f"Saved context counts for {year} to {output_file}")

def count_context_words_sequential(target, start_year, end_year, directory, window_size=10, top_n=2000, output_dir="/home/maxzhuyt/Desktop/coha/context_counts"):
    # Process each year sequentially, saving each result immediately
    for year in range(start_year, end_year + 1):
        process_year(target, year, directory, window_size, top_n, output_dir)


def combine_yearly_csvs(target, start_year, end_year, output_dir, combined_output_file):
    ...
    for year in range(start_year, end_year + 1):
        yearly_file = os.path.join(output_dir, f"context_counts_{target}_{year}.csv")
...
if __name__ == "__main__":
    # Arguments: target word, start year, end year
    if len(sys.argv) != 4:
        print("Usage: python count_context_words.py <target_word> <start_year> <end_year>")
        sys.exit(1)
    ...
    output_dir = f"/scratch/midway3/maxzhuyt/context_counts/{target_word}"
    combined_output_file = f"/scratch/midway3/maxzhuyt/context_counts/{target_word}_combined_{start_year}_{end_year}.csv"
    os.makedirs(output_dir, exist_ok=True)
    print(f"Counting context words for '{target_word}' from {start_year} to {end_year}...")

    # Generate and save the normalized counts for each year sequentially
    count_context_words_sequential(target_word, start_year, end_year, directory, output_dir=output_dir)
    combine_yearly_csvs(target_word, start_year, end_year, output_dir, combined_output_file)
